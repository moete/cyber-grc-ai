...
export async function up(db: Kysely<any>): Promise<void> {
  await db.schema
    ...
    .addColumn('action', 'varchar(50)', (col) => col.notNull().check(sql`action IN ('CREATE', 'UPDATE', 'DELETE')`))
  ...
  await db.schema.createIndex('idx_audit_logs_organization_id').on('audit_logs').column('organization_id').execute();

  await db.schema.createIndex('idx_audit_logs_entity').on('audit_logs').columns(['entity_type', 'entity_id']).execute();
