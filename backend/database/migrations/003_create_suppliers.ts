...
export async function up(db: Kysely<any>): Promise<void> {
  await db.schema
    ...
    .addColumn('category', 'varchar(50)', (col) => col.notNull().check(sql`category IN ('SaaS', 'Infrastructure', 'Consulting', 'Other')`))
  ...
  await db.schema.createIndex('idx_suppliers_organization_id').on('suppliers').column('organization_id').execute();

  await db.schema.createIndex('idx_suppliers_category').on('suppliers').column('category').execute();

  await db.schema.createIndex('idx_suppliers_risk_level').on('suppliers').column('risk_level').execute();
}
