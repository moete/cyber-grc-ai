 ...
 */
export async function isEmailTakenInOrg(organizationId: string, email: string, excludeUserId?: string): Promise<boolean> {
  let q = scopedSelect('users', organizationId).where('email', '=', email.toLowerCase()).select('id');
  if (excludeUserId) {
 ...
 */
export async function deleteUser(organizationId: string, userId: string, currentUserId: string): Promise<void> {
  ...
  if (user.role === 'Owner') {
    const owners = await scopedSelect('users', organizationId).where('role', '=', 'Owner').select('id').execute();
    if (owners.length <= 1) {
