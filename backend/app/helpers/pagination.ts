...
export async function countQuery(query: any): Promise<number> {
  const result = await query.select((eb: any) => eb.fn.countAll().as('count')).executeTakeFirst();
 ...
 */
export function paginated<T>(response: Response, data: T[], pg: PaginationParams, total: number) {
