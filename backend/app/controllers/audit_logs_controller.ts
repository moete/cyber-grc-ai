...
export default class AuditLogsController {
  ...
  async index({ auth, request, response }: HttpContext) {
    ...
    const rows: AuditLogRow[] = await query.selectAll().orderBy('created_at', 'desc').limit(pg.limit).offset(pg.offset).execute();
  ...
  async show({ auth, params, response }: HttpContext) {
    const log = (await scopedSelect('audit_logs', auth.organizationId).where('id', '=', params.id).selectAll().executeTakeFirst()) as
      | AuditLogRow
      | undefined;
